...
import com.exakt.vvip.service.UserManagementService;
...
public class UserManagementController {
    ...
    public ResponseEntity<List<UserResponse>> getByBranch(@PathVariable Long branchId) {
        return ResponseEntity.ok(userManagementService.getByBranch(branchId));
    }

    @GetMapping("/by-manager/{managerId}")
    @Operation(summary = "Get agents under a manager")
    public ResponseEntity<List<UserResponse>> getByManager(@PathVariable Long managerId) {
        return ResponseEntity.ok(userManagementService.getByManager(managerId));
    }

    @GetMapping("/by-role/{role}")
    @Operation(summary = "Get users by role (ADMIN, MANAGER, AGENT)")
